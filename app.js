...
function renderPaper(){
  ...
  const active=$("activePortfolios");
  if(active)active.innerHTML=ps.length?ps.map(p=>'<div class="active-portfolio"><strong>'+escapeHTML(p.symbol)+'</strong> · תיק פעיל<small>'+escapeHTML(names[p.plan]||p.plan||'—')+' · '+(Number(p.units)>0?'מחזיק מניות':'ללא החזקה כרגע')+'</small><small>'+escapeHTML(automationText(p))+'</small></div>').join(''):'<span class="muted">אין תיקים פעילים.</span>';
  ...
  $("position").innerHTML=ps.map(p=>{const v=positionValue(p),pe=p.plan==="ai_dynamic"?aiTargetExposure((latestSnap(p)?.score)??(market?.symbol===p.symbol?a?.master:50)):(exp[p.plan]||.5);return `<tr><td><b class="green">${escapeHTML(p.symbol)}</b><span class="active-label">● תיק פעיל</span>${p.predecessorId?'<div class="muted">נפתח במעבר אוטומטי</div>':''}</td><td>${names[p.plan]||p.plan||"—"}<div class="muted">${automationText(p)}</div></td><td>${Math.round(pe*100)}%</td><td>${money(p.allocatedILS)}</td><td><span class=ltr>$${Number(p.allocatedUSD).toFixed(2)}</span></td><td>${Number(p.entryFX).toFixed(4)}</td><td><span class=ltr>$${Number(p.entryPrice).toFixed(2)}</span></td><td><span class="ltr ${v.px>=Number(p.entryPrice)?"green":"red"}">$${Number(v.px).toFixed(2)}</span></td><td><span class=ltr>${Number(p.units).toFixed(4)}</span></td><td>${money(v.marketILS)}</td><td class="${v.pnl>=0?"green":"red"}">${v.pnl>=0?"+":""}${money(v.pnl)}</td></tr>`}).join("");
}
