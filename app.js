...
let market=null,fx=null,a=null,plan=localStorage.getItem("v4plan")||"balanced";const exp={conservative:.25,balanced:.5,growth:.8},names={conservative:"שמרני",balanced:"מאוזן",growth:"צמיחה"};
const sleep=ms=>new Promise(r=>setTimeout(r,ms));
const CACHE_MARKET_MS=30*60*1000,CACHE_FX_MS=12*60*60*1000,STALE_FX_MS=7*24*60*60*1000;
...
function shortError(msg=""){msg=String(msg);if(/25 requests per day|free API requests|rate limit|premium plans|1 request per second/i.test(msg))return "מגבלת Alpha Vantage החינמית הופעלה. המערכת תשתמש בנתונים שמורים כשאפשר.";if(/API key/i.test(msg))return "מפתח ה‑API אינו מוגדר ב‑Netlify.";if(/FX|USD\/ILS|Currency/i.test(msg))return "לא ניתן לקבל כרגע שער דולר/שקל.";return "לא ניתן לטעון כרגע את נתוני השוק. נסה שוב בעוד זמן קצר."}
...
async function fetchJson(url){
  const r=await fetch(url,{cache:"no-store"}),j=await r.json().catch(()=>({}));
...
async function load(){
  try{
    ...
    const marketKey="v41_market_"+symbol;
    const fxKey="v41_fx_usdils";

    market=cacheGet(marketKey,CACHE_MARKET_MS);
    let marketFromCache=!!market;
    if(!market){
      market=await fetchJson(`${APP_CONFIG.marketEndpoint}?symbol=${symbol}`);
      cacheSet(marketKey,market);
    }

    fx=cacheGet(fxKey,CACHE_FX_MS);
    let fxFromCache=!!fx;
    if(!fx){
      // Alpha Vantage free tier allows only one request per second.
      // Wait before the second upstream request when market data was just fetched.
      if(!marketFromCache) await sleep(1300);
      try{
        fx=await fetchJson(APP_CONFIG.fxEndpoint);
        cacheSet(fxKey,fx);
      }catch(err){
        const stale=cacheGet(fxKey,STALE_FX_MS);
        if(stale) fx=stale;
        else throw err;
      }
    }
    ...
    $("price").innerHTML=`<span class=ltr>$${a.last.toFixed(2)}</span>`;
    $("marketDate").textContent=market.lastRefreshed+(marketFromCache?" · שמור":"");
    $("fx").textContent=Number(fx.rate).toFixed(4);
    $("fxDate").textContent=(fx.lastRefreshed||"")+(fxFromCache?" · שמור":"");
    $("master").textContent=a.master+"/100";
    ...
    $("status").textContent="שגיאת טעינה";
  }
}

function paper(){return JSON.parse(localStorage.getItem("v4paper")||"null")}function save(p){localStorage.setItem("v4paper",JSON.stringify(p));renderPaper()}
...
$("load").onclick=load;$("open").onclick=openPaper;$("snapshot").onclick=snapshot;$("close").onclick=closeP;$("reset").onclick=()=>{if(confirm("לאפס את תיק הנייר ואת ה-Snapshots?")){localStorage.removeItem("v4paper");localStorage.removeItem("v4snaps");renderPaper();renderHistory()}};$("plan").textContent=names[plan];$("expo").textContent=exp[plan]*100+"% חשיפה";renderPaper();renderHistory();window.addEventListener("resize",()=>draw(snapshots()));