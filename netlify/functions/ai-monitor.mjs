...
export default async function monitor(){
  ...
  try{
    ...
    if(symbols.length&&open){
      ...
      const fxData=loaded[0].value,quotes=alignSeries(Object.fromEntries(universe.map((symbol,i)=>[symbol,loaded[i+1].value])));
      result=await db('rpc/apply_paper_cycle',{method:'POST',body:JSON.stringify({p_slot:slot(),p_fx:Number(fxData.price),p_quotes:quotes})});
      symbols=result.symbols||symbols;
    }
  }catch(e){errors.push(e.message);}
  const waiting=errors.some(e=>/ממתין לנתונים מסונכרנים|ממתין למכסת|API credits/i.test(e));
