...
async function td(endpoint,q={}){
  ...
  if(!r.ok||d?.status==="error"||d?.code>=400){
    throw Error(d?.message||`Twelve Data HTTP ${r.status}`);
  }
  return d;
}
...
function calc(pr){
  ...
  const s20=mean(c.slice(0,20));
  const s60=mean(c.slice(0,60));
  const rs=[];
  ...
  return {
    last,marketS,trend,risk,mom,master,
    signal:master>=70?"חיובי":master>=55?"חיובי מתון":master>=45?"ניטרלי":master>=30?"זהירות":"שלילי"
  };
}

function target(score){
  return score>=70?.80:score>=55?.65:score>=45?.50:score>=30?.35:.25;
}

function slot6(d=new Date()){
  const z=new Date(d);
  z.setUTCSeconds(0,0);
  z.setUTCMinutes(Math.floor(z.getUTCMinutes()/6)*6);
  return z.toISOString().slice(0,16).replace(/[:T]/g,"-");
}
...
async function heartbeat(patch){
  ...
  await db("agent_monitor_status?id=eq.1",{
    method:"PATCH",
    body:JSON.stringify({
      ...patch,
      checks,
...
export default async()=>{
  try{
    ...
    const ps=(rows||[])
      .filter(p=>["SPY","QQQ"].includes(String(p.symbol).toUpperCase()))
      .slice(0,2);
    ...
    if(!ps.length){
      await heartbeat({
        status:"idle",
        symbols:[],
        note:"אין תוכניות SPY/QQQ פתוחות"
      });
      return;
    }

    const fxData=await td("price",{symbol:"USD/ILS"});
    const fx=Number(fxData?.price);
    ...
    for(const p of ps){
      ...
      if(Number.isFinite(current)&&pr.length){
        pr[0]={...pr[0],close:current};
      }

      const latest=pr[0];
      const a=calc(pr);
      marketDates.push(latest.date);

      const actual=(+p.start)>0?(+p.allocated_ils)/(+p.start):null;
      const mode=p.strategy_mode||p.plan;
      const te=mode==="ai_dynamic"
        ? target(a.master)
        : ({conservative:.25,balanced:.5,growth:.8}[mode]||.5);

      const action=mode==="ai_dynamic"
        ? (te>(actual+.02)?"הגדלת חשיפה":te<(actual-.02)?"הקטנת חשיפה":"ללא שינוי")
        : "מסלול קבוע";
      ...
      // A separate simulated observation every 6-minute slot.
      const key=`monitor-${p.id}-${p.symbol}-${slot6()}`;
      const ex=await db(
        `portfolio_snapshots?select=id&snapshot_key=eq.${encodeURIComponent(key)}&limit=1`
      );

      if(!ex?.length){
        const pv=(+p.units)*a.last*fx;
        const ppnl=pv-(+p.allocated_ils);
        ...
        await db("portfolio_snapshots",{
          ...
          body:JSON.stringify({
            ...
            market_price:a.last,
            auto:true,
            market_score:a.marketS,
            trend_score:a.trend,
            risk_score:a.risk,
            momentum_score:a.mom,
            recommendation:a.signal,
            plan:mode||null,
            target_exposure:te,
            actual_exposure:actual,
    ...
    await heartbeat({
      ...
      last_market_date:marketDates.sort().reverse()[0]||null,
      scores,
      note:"Twelve Data · בדיקה אוטומטית כל 6 דקות · 80 דגימות ביום · סימולציה בלבד"
    });

  }catch(e){
    console.error("AI_MONITOR_ERROR",e?.stack||String(e));
    try{
      await heartbeat({status:"error",note:String(e?.message||e)});
    }catch{}
    throw e;
  }
};
...
// 80 runs/day × about 5 Twelve Data credits = about 400 credits/day.
