...
export function validateMarket(d,now=Date.now()){
 ...
 if(d.timeframeSeconds!==300)throw Error('M5 collector required');
...
export function analyzeMarket(state,now=Date.now()){
 ...
 const target=Math.floor(now/300000)*300-300,blocked=[],candidates=[];
 let observed=0;
 for(const symbol of catalog){
  const x=items[symbol];
  let reason='';
  if(!x||x.timeframeSeconds!==300||now/1000-x.capturedAt>600||x.capturedAt>now/1000+5)reason='ממתין לעדכון במחזור האיסוף';
  else {
   observed++;
   if(!x.tradeAllowed)reason='הברוקר אינו מאפשר מסחר בנכס';
    ...
    if(i<0||x.bars.length-i<25)reason='ממתין ל־25 נרות ולנר 5 דקות סגור משותף';
    else {
 ...
 return {source:'colmex-mt4',readOnly:true,account:'102598',server:'TGLColmex-Demo',catalogCount:catalog.length,totalAvailable:state?.totalAvailable||0,observed,candidates,blocked,barTime:target,complete:catalog.length>0&&observed===catalog.length,universeId:'M5:'+fingerprint(candidates.map(x=>x.symbol)),receivedAt:state?.received_at||null};
}
export function confirmation(scans){
 const first=scans[0];let consecutive=0;
 if(first)for(let i=0;i<Math.min(scans.length,3);i++){
  const s=scans[i];if(s.leader!==first.leader||s.score<60||s.universe_id!==first.universe_id||Number(first.bar_time)-Number(s.bar_time)!==i*300)break;
  consecutive++;
 }
