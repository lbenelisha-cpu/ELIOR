...
export function exportContracts(items,now=Date.now()){
 return {format:'elior-atrade-contracts-v1',exportedAt:new Date(now).toISOString(),purpose:'אימות מפרטי חוזים לפני סימולציה; ללא מפתחות או מידע על פוזיציות',contracts:Object.values(items||{}).map(x=>Object.fromEntries(fields.filter(k=>x[k]!==undefined).map(k=>[k,x[k]])))};
}
