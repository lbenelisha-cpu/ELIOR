export const WATCHLIST = ['SPY','QQQ','DIA','IWM','AAPL','MSFT','NVDA','AMZN','GOOGL','META','TSLA','JPM','GLD','SLV','USO','DBA','TLT','IEF','VNQ','EFA','EEM','UUP','IBIT','ETHA'];
...
export async function td(endpoint,params={},cacheSeconds=1800) {
 if(![60,1800].includes(cacheSeconds))throw Error("Invalid cache duration");
 const cacheSlot=new Date(Math.floor(Date.now()/(cacheSeconds*1000))*cacheSeconds*1000).toISOString();
 const key=endpoint+':'+JSON.stringify(Object.entries(params).sort(([a],[b])=>a.localeCompare(b)))+':'+cacheSlot+(cacheSeconds===1800?'':':ttl60');
 ...
 try {
  ...
  await db('paper_market_cache?on_conflict=key',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=representation'},body:JSON.stringify({key,payload,expires_at:new Date(Date.parse(cacheSlot)+cacheSeconds*1000).toISOString(),lease_until:null})});
  return payload;
 }catch(e){
