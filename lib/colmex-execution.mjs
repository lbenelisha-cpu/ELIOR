...
// Broker descriptions identify stock CFDs; do not classify indexes or commodities as shares.
export const isStock=x=>!!x&&x.profitMode===1&&x.contractSize===1&&/^CFD /i.test(x.description||'')&&!/index|indices|oil|gas|gold|silver|cocoa|coffee|bond|ETF/i.test(x.description||'');
...
export function planCycle(snapshot,market,scans=[],closedLedger=[],now=Date.now()/1000,recentCommands=[],initialEntry=false){
 ...
 for(const p of programs){
  ...
  const firstEntry=initialEntry&&!p.orders.length&&![...allClosed.values()].some(o=>o.magic===CONFIG.magics[p.id-1])&&!recentCommands.some(c=>c.command?.program===p.id&&c.command?.action==='BUY'&&c.status==='filled');
  ...
  if(!reason&&p.orders.length&&(p.exposure>p.cap+.01||p.cap<=0||totalExposure>CONFIG.capital+.01)){
   ...
   if(!analysis.complete)reason='ממתין למחזור נתונים מלא';
    ...
    candidate=eligible.find(c=>c.symbol!==other.symbol&&!reservedCandidates.has(c.symbol)&&(!firstEntry||isStock(items[c.symbol]))&&c.master>=60&&quoteSpec(items[c.symbol],items,now).minimum<=p.target);
    if(candidate)reservedCandidates.add(candidate.symbol);
    const current=analysis.candidates.find(c=>c.symbol===p.symbol);
    if(!p.orders.length){
     if(candidate){action='BUY';symbol=candidate.symbol;const rotation=recentCommands.find(x=>x.status==='filled'&&x.command.program===p.id&&x.command.purpose==='rotation'&&x.command.barTime===analysis.barTime&&x.command.destination===symbol);key=(rotation?'ROTATE:':'ENTRY:')+symbol;reason=firstEntry?'כניסה ראשונית למניה ללא המתנה לאימותים':rotation?'השלמת מעבר לאחר אישור המכירה בדמו':'כניסה לאחר שלושה אימותים';}
  ...
  if(!command&&!commonReason&&(confirmed>=3||firstEntry&&action==='BUY')&&action!=='HOLD'){
   if(action==='BUY'){
    ...
    else command=add('BUY',{symbol},lots,firstEntry?'initial-entry':'entry-or-rebalance');
   }else {
  ...
  p.initialEntry=firstEntry;p.status=reason;p.confirmations=confirmed;p.candidate=candidate?.symbol||null;
