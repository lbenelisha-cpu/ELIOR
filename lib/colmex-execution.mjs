import {brokerRate,validRate} from './colmex-fx.mjs';
import {analyzeMarket,fingerprint} from './colmex-market.mjs';
export const CONFIG=Object.freeze({account:'102598',server:'TGLColmex-Demo',capital:10000,budget:5000,target:.8,minimumScore:60,advantage:10,confirmations:3,band:.02,minimumILS:100,magics:[31025981,31025982]});
...
export function quoteSpec(x,items,now=Date.now()/1000,{forEntry=true}={}){
 if(!x)return {ok:false,reason:'לא התקבלו נתוני הנכס מ־MT4'};
 if(!safeSymbol(x.symbol)||x.depositCurrency!=='USD'||![0,1].includes(x.profitMode))return {ok:false,reason:'שיטת חוזה או מטבע חשבון אינם נתמכים'};
...
function streak(scans,current,program,key){
 let n=1;for(let i=1;i<3;i++){const prev=scans.find(s=>Number(s.bar_time)===current.barTime-i*1800)?.scan;if(!prev||prev.universe!==current.universe||prev.programs?.[program-1]?.key!==key)break;n++;}return n;
}
export function planCycle(snapshot,market,scans=[],closedLedger=[],now=Date.now()/1000,recentCommands=[],thresholdFX=null){
 ...
 const universe=fingerprint(eligible.map(x=>x.symbol)),scan={barTime:analysis.barTime,universe,programs:[]},commands=[];
 const fx=brokerRate(items,now)||(validRate(thresholdFX,now)?thresholdFX:null);
 const minTurnover=fx?CONFIG.minimumILS/fx.rate:null;
 let commonReason='';
 if(snapshot.historyOverflow)commonReason='היסטוריית עסקאות גדולה ממגבלת הסנכרון';
 else {
  const reasons=programs.flatMap(p=>[
   ...p.valued.filter(v=>!v.spec.ok).map(v=>{
    const x=items[v.order.symbol],age=finite(x?.tickTime)?Math.max(0,Math.floor(now-x.tickTime)):null;
    return 'תוכנית '+p.id+' · '+v.order.symbol+': '+v.spec.reason+(age===null?' · זמן ציטוט חסר':' · גיל מחיר '+age+' שניות (מגבלה 90)')+(finite(x?.capturedAt)?' · גיל איסוף '+Math.max(0,Math.floor(now-x.capturedAt))+' שניות (מגבלה 180)':' · זמן איסוף חסר');
   }),
   ...(new Set(p.orders.map(o=>o.symbol)).size>1?['תוכנית '+p.id+': קיימים מספר נכסים בתוכנית']:[])
  ]);
  if(reasons.length)commonReason='פעולות שתי התוכניות ממתינות — '+reasons.join(' | ');
 }
 ...
 for(const p of programs){
  ...
  if(!reason&&p.orders.length&&(p.exposure>p.cap+.01||p.cap<=0||totalExposure>CONFIG.capital+.01)){
   ...
   else if(!fx)reason='ממתין לשער USD/ILS בן עד 60 שניות לצורך סף הפעולה (100 ₪) · '+(thresholdFX?.error||'אין שער תקין');
   else {
