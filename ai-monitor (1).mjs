...
export default async()=>{try{
  ...
  for(const p of ps){
    ...
    // A separate simulated observation every 6-minute slot. No real trade is executed and units are never changed.
    const key=`monitor-${p.id}-${p.symbol}-${slot6()}`,ex=await db(`portfolio_snapshots?select=id&snapshot_key=eq.${encodeURIComponent(key)}&limit=1`);
  ...
  await heartbeat({status:"ok",symbols:ps.map(p=>p.symbol),last_market_date:marketDates.sort().reverse()[0]||null,scores,note:"Twelve Data · בדיקה אוטומטית כל 6 דקות בחלון המסחר · 80 דגימות ביום · סימולציה בלבד"});
...
// 80 checks/day: every 6 minutes, Monday-Friday, 14:00-21:59 UTC.
// Each run is about 5 Twelve Data credits for SPY + QQQ + USD/ILS => about 400/day, below the Basic 800/day limit.
