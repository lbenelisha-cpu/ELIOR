...
exports.handler=async event=>{
  ...
  try{
    ...
    if(action==="health"){
      const result={
        env:{supabaseUrl:!!SUPABASE_URL,serviceRoleKey:!!SERVICE_KEY},
        urlHost:null,paperTable:false,snapshotsTable:false
      };
    ...
    if(action==="get"){
      ...
      let monitor=null;
      try{const m=await sb("agent_monitor_status?select=*&id=eq.1&limit=1");monitor=m?.[0]||null}catch(e){console.warn("MONITOR_STATUS_UNAVAILABLE",e.message)}
      ...
      return{statusCode:200,headers,body:JSON.stringify({portfolio:portfolios[0]||null,portfolios,snapshots:s||[],monitor})};
    }
