(() => {
 const root=document.getElementById('atrade-panel');if(!root)return;
 root.innerHTML=`<h2>אטרייד · נכסים לניתוח</h2><p>נתונים ישירות מ־MT4. הסוכנים מדרגים לפי נרות חצי־שעתיים סגורים; כרגע אין שינוי בתיק הכספי ואין פקודות לברוקר.</p>
 <div class="actions"><label for="atrade-key">מפתח צפייה של MT4 </label><input id="atrade-key" type="password" autocomplete="off"><button id="atrade-connect" type="button">התחבר לאטרייד</button><button id="atrade-disconnect" type="button" class="secondary">נתק</button><button id="atrade-export" type="button" class="secondary" disabled>הורד מפרטי חוזים לבדיקה</button></div>
 <p id="atrade-status" role="status">ממתין לחיבור באמצעות מפתח הקריאה הקיים.</p><div id="atrade-summary"></div>
 ...
 <label for="atrade-filter">חיפוש נכס </label><input id="atrade-filter" type="search" placeholder="שם או סמל אצל הברוקר">
 <div class="table"><table><thead><tr><th>נכס</th><th>תיאור</th><th>ציון</th><th>שוק / מגמה / סיכון / מומנטום</th><th>מרווח %</th><th>בדיקת חוזה</th></tr></thead><tbody id="atrade-rows"></tbody></table></div>
 <div class="actions"><button id="atrade-prev" type="button" class="secondary">הקודם</button><span id="atrade-page"></span><button id="atrade-next" type="button" class="secondary">הבא</button></div>
 <details><summary>10 סריקות אטרייד אחרונות</summary><div id="atrade-history"></div></details>
 <details><summary>נכסים שממתינים לנתונים</summary><div id="atrade-blocked"></div></details>
 <p class="muted">המפתח נשמר בזיכרון הלשונית בלבד ונמחק בניתוק או בסגירה. <a href="/atrade-market.html">פתיחת עמוד אטרייד המלא</a></p>`;
 const el=id=>document.getElementById('atrade-'+id),esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
 ...
 async function refresh(){
  ...
  try{const r=await fetch('/.netlify/functions/atrade-market',{headers:{Authorization:'Bearer '+token},cache:'no-store',signal:c.signal});const d=await r.json();if(g!==generation)return;if(!r.ok)throw Error(d.error||'לא ניתן לקרוא נתונים');
 ...
 el('disconnect').onclick=()=>{generation++;controller?.abort();controller=null;token='';clear();el('status').textContent='אטרייד נותק מהתצוגה';};
 ...
 el('export').onclick=async()=>{
  ...
  try{const r=await fetch('/.netlify/functions/atrade-market?action=contracts',{headers:{Authorization:'Bearer '+token},cache:'no-store',signal:AbortSignal.timeout(15000)});const d=await r.json();if(g!==generation)return;if(!r.ok)throw Error(d.error||'הייצוא נכשל');
   const url=URL.createObjectURL(new Blob([JSON.stringify(d,null,2)],{type:'application/json'})),a=document.createElement('a');a.href=url;a.download='atrade-contracts.json';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);
